...
function drawCircle(clients) {
  ...
  .text("诸侯讨董")
  d3.select("p")
  .text("中平六年（189年），董卓废嫡立庶，残暴专权。袁绍，曹操等中央官员纷纷逃离京都，联合关东群雄，于初平元年（190年）组成讨董联军，进军洛阳，经过多场战役，双方各有胜负。而后董卓迁都长安，火烧洛阳。虽然讨董联军声势浩大，各方诸侯却惧怕董卓，各自保存实力。伴随着一系列的内讧，讨董联盟于初平二年（191年）瓦解。")
  ...
  d3.select("#circle1").on('click', function() {
    ...
    .text("群雄割据")
    d3.select("p")
    .text("讨董联盟瓦解之后，王允设计，挑拨吕布杀死董卓。之后董卓残部击败王允吕布，重新占领长安。汉献帝再次沦为傀儡，中央对地方的控制愈发微弱。地方诸强相互征伐，最终形成了群雄割据的局。")
  ...
  d3.select("#circle2").on('click', function() {
    ...
    .text("官渡之战")
    d3.select("p")
    .text("曹操挟天子以令诸侯，先后占据了兖州、徐州、司隶以及豫州、扬州部分地区。袁绍在打败公孙瓒之后坐拥幽州、冀州、青州、并州之众，结连乌桓，尽有黄河以北之地，意欲南向以争天下。建安五年（200年），曹操军与袁绍军相持于官渡，在此展开战略决战。曹操奇袭袁军在乌巢的粮仓，继而击溃袁军主力。此战奠定了曹操统一中国北方的基础。")
  ...
  d3.select("#circle3").on('click', function() {
    ...
    .text("赤壁之战")
    d3.select("p")
    .text("建安十三年（208年），基本控制北方的曹操率大军南下荆州。荆州牧刘琮投降。寄身荆州的刘备逃亡江夏，与江东孙吴协议结盟。孙权派遣都督周瑜、程普率军与刘备组成联军，在赤壁大破曹军，曹操落荒北回。此战后，曹军退守襄阳，曹孙刘三分荆州，三国鼎立的雏形由此建立。")
  ...
  d3.select("#circle4").on('click', function() {
    ...
    .text("刘备入蜀")
    d3.select("p")
    .text("赤壁之战后，刘备占据荆南五郡，已有意图占据益州。益州牧刘璋接受部下建议，邀刘备入蜀攻打张鲁，抵御曹操。建安十七年（212年），刘璋与刘备决裂并发动战争。建安十九年（214年），刘备围困成都，刘璋向刘备投降。刘备自此夺得益州，实现了诸葛亮隆中对中「跨有荆、益」的战略规划。")
  ...
  d3.select("#circle5").on('click', function() {
    ...
    .text("三国鼎立")
    d3.select("p")
    .text("220年曹操之子曹丕逼迫汉献帝刘协禅让帝位，登基建立魏国。接下来的数年内，占据蜀地的刘备与盘踞东南孙权相继称帝，三国鼎立的时代开始。")
  ...
  d3.select("#circle6").on('click', function() {
    ...
    .text("曹魏灭蜀")
    d3.select("p")
    .text("蜀汉建立后，经过一系列的北伐，国力日渐消耗，加上后主刘禅宠信宦官，朝政日非。263年，曹魏向蜀汉发动战争，分三路进攻汉中。最终邓艾率精兵偷渡阴平攻占涪城，进逼成都。汉后主刘禅出降，蜀汉灭亡。")
  ...
  d3.select("#circle7").on('click', function() {
    ...
    .text("司马炎篡魏")
    d3.select("p")
    .text("司马昭死后，其子司马炎继承相国晋王位，掌握全国军政大权。经过精心准备，魏咸熙二年（266年2月8日），司马炎仿效当年曹丕篡汉的故事，逼迫魏元帝禅让，即位为帝。晋朝154年国祚由此展开。")
